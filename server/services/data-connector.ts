...
class DataConnectorService {
  ...
  constructor() {
    ...
    console.log("Airbyte config:", {
      ...
      workspaceId: process.env.WORKSPACE_ID || "missing",
    });
  }
  ...
  async setupConnectors(): Promise<{
    ...
    try {
      ...
      const connectors = sourceDefinitions.data
        .filter((def: any) =>
          supportedConnectors.some((name) => def.name.includes(name)),
        )
        .map((def: any) => ({
      ...
      return {
        success: false,
        error: `Failed to setup data connectors: ${error}`,
      };
    }
  }
  ...
  async createConnector(config: ConnectorConfig): Promise<ConnectorResponse> {
    try {
      ...
      if (!workspaceId) {
        throw new Error("Workspace ID environment variable is required");
      }

      // Step 1: Create source using correct Airbyte Cloud API
      const source = await this.makeApiCall("/sources", {
        method: "POST",
        body: JSON.stringify({
          workspaceId,
          name: `${config.service}_source_${Date.now()}`,
          definitionId: this.getSourceDefinitionId(config.service),
          configuration: config.config,
        }),
      });

      // Step 2: Get or create Snowflake destination
      const destinations = await this.makeApiCall("/destinations", {
        method: "GET",
      });

      let destinationId = destinations.data?.find(
        (dest: any) =>
          dest.workspaceId === workspaceId && dest.name.includes("Snowflake"),
      )?.destinationId;
      ...
      if (!destinationId) {
        const destination = await this.makeApiCall("/destinations", {
          ...
          body: JSON.stringify({
            ...
            definitionId: this.getSnowflakeDestinationId(),
            configuration: {
      ...
      const connection = await this.makeApiCall("/connections", {
        ...
        body: JSON.stringify({
          ...
          destinationId,
          name: `${config.service}_connection_${Date.now()}`,
          dataResidency: "auto",
        }),
      });
  ...
  async getConnectorStatus(connectionId: string): Promise<ConnectorResponse> {
    try {
      ...
      const jobs = await this.makeApiCall(
        `/jobs?connectionId=${connectionId}&limit=1`,
      );
      ...
      return {
        success: false,
        error: `Failed to get connector status: ${error}`,
      };
    }
  }
  ...
  async listConnectors(
    ...
    try {
      ...
      const connections = await this.makeApiCall(
      ...
      const connectors = await Promise.all(
        connections.data.map(async (conn: any) => {
          const jobs = await this.makeApiCall(
            `/jobs?connectionId=${conn.connectionId}&limit=1`,
          );
          const lastJob = jobs.data?.[0];
