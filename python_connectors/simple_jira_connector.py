"""
Simplified Jira API connector without pandas dependency.
"""
...
class SimpleJiraConnector(SimpleBaseConnector):
    """Simplified Jira API connector using REST API v3"""
    ...
    def __init__(self, company_id: int, credentials: Dict[str, Any], config: Dict[str, Any] = None):
        ...
        self.default_objects = [
            ...
            "issue_types", "components", "versions"
        ]
    ...
    def make_paginated_request(self, url: str, params: Dict[str, Any] = None) -> List[Dict]:
        ...
        all_results = []
        start_at = 0
        max_results = 50  # Smaller batch size for reliability
        
        if params is None:
            params = {}
        
        max_pages = 10  # Limit to prevent issues
        page_count = 0
        ...
        while page_count < max_pages:
            ...
            try:
                response = requests.get(url, headers=self.get_headers(), params=request_params, timeout=30)
                response.raise_for_status()
    ...
    def extract_issues(self, incremental: bool = True) -> List[Dict[str, Any]]:
        """Extract Jira issues"""
        try:
            ...
            if incremental:
                # Get issues updated in last 30 days for demo
            ...
            params = {
                'jql': jql,
                'fields': 'id,key,summary,status,assignee,reporter,created,updated,priority,issuetype'
            }
            ...
            logger.info(f"Extracted {len(flattened_issues)} issues")
    ...
    def extract_projects(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(flattened_projects)} projects")
    ...
    def extract_generic_object(self, object_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(results)} {object_name}")
            return results
