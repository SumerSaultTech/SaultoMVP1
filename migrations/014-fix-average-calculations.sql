-- Migration to fix average metric calculations to use proper cumulative averages
-- This updates the ETL function to handle average metrics correctly by calculating 
-- cumulative averages instead of summing daily averages
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Calculated daily goal for %: % (type: %, yearly: %, is_average: %)', 
      metric_record.label, daily_goal, COALESCE(kpi_record.goal_type, 'unknown'), kpi_yearly_goal, is_average_metric;
    ...
    LOOP
      ...
      IF is_average_metric THEN
        -- For averages: Calculate cumulative average across all records in period
        -- Only count records where there were actual resolved issues (daily_actual > 0)
        IF daily_actual > 0 THEN
          total_records := total_records + 1;
          running_actual := ((running_actual * (total_records - 1)) + daily_actual) / total_records;
        ELSE
          -- If no issues resolved on this day, running average stays the same
          running_actual := running_actual;
        END IF;
        
        -- For goal calculation with averages, we still use cumulative approach
      ...
      -- Insert goal values (calculated proportionally for averages too)
      EXECUTE format('
        INSERT INTO %I.metrics_time_series 
        (ts, metric_key, series_label, value, running_sum, is_goal, period_type,
         period_start, period_end, period_relative_value, period_relative_running_sum, period_baseline_value, updated_at)
    ...
    RAISE NOTICE 'Completed metric: % with % actual, % goal running sums (daily goal: %, total records: %)', 
      metric_record.label, running_actual, running_goal, daily_goal, total_records;
  END LOOP;
