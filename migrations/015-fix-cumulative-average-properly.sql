-- Fix cumulative average calculation to properly calculate across all individual records
-- The previous approach was still averaging averages, we need to track individual records
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Completed metric: % with % actual, % goal running sums (daily goal: %, total records: %)', 
      metric_record.label, running_actual, running_goal, daily_goal, cumulative_count;
  END LOOP;
