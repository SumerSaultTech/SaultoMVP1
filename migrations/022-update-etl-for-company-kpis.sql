-- Update ETL function to use company-specific KPI tables instead of public.kpi_metrics
-- This completes the multi-tenancy fix by making ETL use isolated KPI definitions
...
DECLARE
  ...
  kpi_found BOOLEAN := false;  -- Track if KPI was found
  kpi_goal_type TEXT;
...
BEGIN
  ...
  LOOP
    ...
    goal_target := 0;
    daily_goal_value := 0;
    kpi_found := false;  -- Reset KPI found flag
    
    -- Check if this is an average-type metric
    is_average_metric := metric_record.expr_sql ILIKE '%AVG%';
    
    -- UPDATED: Get goal information from COMPANY-SPECIFIC kpi_metrics table (not public)
    BEGIN
      EXECUTE format('
        SELECT yearly_goal, goal_type, quarterly_goals, monthly_goals, true
        FROM %I.kpi_metrics 
        WHERE (LOWER(name) = LOWER($1) 
               OR name = $1
               OR TRIM(LOWER(name)) = TRIM(LOWER($1)))
        LIMIT 1
      ', schema_name) 
      INTO kpi_yearly_goal, kpi_goal_type, kpi_quarterly_goals, kpi_monthly_goals, kpi_found
      USING metric_record.label;
    EXCEPTION 
      WHEN OTHERS THEN
        kpi_found := false;
        RAISE NOTICE 'KPI lookup failed for %: %', metric_record.label, SQLERRM;
    END;
    
    -- Debug KPI lookup
    IF kpi_found THEN
      RAISE NOTICE 'Found KPI for % in company schema: yearly_goal=%, goal_type=%', 
        metric_record.label, kpi_yearly_goal, kpi_goal_type;
    ELSE
      RAISE NOTICE 'WARNING: No KPI found for metric % in company schema %', 
        metric_record.label, schema_name;
    END IF;
    
    -- Calculate goal target and daily goal value based on goal type
    IF kpi_found AND kpi_yearly_goal IS NOT NULL AND kpi_yearly_goal::NUMERIC > 0 THEN
      kpi_yearly_goal := kpi_yearly_goal::NUMERIC;  -- Convert to numeric
  ...
  RAISE NOTICE 'Completed ETL for company % period % using company-specific KPI table', company_id_param, period_type_param;
END;
$$;