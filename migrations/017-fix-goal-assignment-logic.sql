-- Fix goal assignment logic for average metrics
-- The issue is that daily_goal is being used for storage but gets overwritten
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Calculated goal for %: target=% daily_goal_value=% (type: %, yearly: %, is_average: %)', 
      metric_record.label, goal_target, daily_goal_value, COALESCE(kpi_record.goal_type, 'unknown'), kpi_yearly_goal, is_average_metric;
