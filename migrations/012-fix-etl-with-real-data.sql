-- Migration to fix ETL function to use real Jira data from core_jira_issues table
-- This replaces the broken ETL function with one that properly calculates metrics from real data
...
BEGIN
  ...
  LOOP
    ...
    SELECT yearly_goal, goal_type, quarterly_goals, monthly_goals
    INTO kpi_record
    FROM kpi_metrics 
    WHERE company_id = company_id_param 
      AND LOWER(name) = LOWER(metric_record.label)
    LIMIT 1;
    
    -- Calculate daily goal based on goal type
    daily_goal := 0;
    IF kpi_record IS NOT NULL AND kpi_record.yearly_goal IS NOT NULL THEN
      yearly_goal := COALESCE(kpi_record.yearly_goal::NUMERIC, 0);
    ...
    RAISE NOTICE 'Calculated daily goal for %: % (type: %, yearly: %)', 
      metric_record.label, daily_goal, COALESCE(kpi_record.goal_type, 'unknown'), yearly_goal;
